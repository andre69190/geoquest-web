...
pl:{
  ...
  badge_beta:"Beta",beta_warning:"Grywalny, ale mogą wystąpić błędy.",
  rotate_device:"Obróć urządzenie poziomo \u{1F4F1}\u27A1\u{1F5FA}"
},
fr:{
...
const MODES=[
  ...
  /* ---- Lifestyle ---- */
  {id:"outline", icon:"\u{1F5FA}",title:"L\u00e4nder-Umrisse",      group:"lifestyle",prompt:"Welches Land hat diese Form?",           desc:"Erkenne L\u00e4nder an ihrer Umrissform",beta:true},
  ...
  {id:"hl_coastline", icon:"\u{1F3D6}",title:"H/L K\u00fcstенl\u00e4nge",group:"hl_compare",prompt:"L\u00e4ngere K\u00fcste?",desc:"Welches Land hat die l\u00e4ngere K\u00fcste?",   comingSoon:true},
  ...
  {id:"flag_fusion",   icon:"\u{1F3C1}",title:"Flaggen-Fusion",        group:"new_modes",prompt:"Welche zwei L\u00e4nder?",                  desc:"Erkenne L\u00e4nder aus verschmolzenen Flaggen",  comingSoon:true},
  {id:"climate_mystery",icon:"\u{1F326}",title:"Klima-Krimi",          group:"new_modes",prompt:"Welches Klima?",                             desc:"Bestimme das Klima eines Landes",                  comingSoon:true},
  {id:"alpha_sprint",  icon:"\u{1F524}",title:"Alphabet-Sprint",       group:"new_modes",prompt:"L\u00e4nder von A\u2013Z",                 desc:"Nenne L\u00e4nder f\u00fcr jeden Buchstaben",     comingSoon:true},
  {id:"wappen_meister",icon:"\u{1F6E1}",title:"Wappen-Meister",    group:"pure_geo",prompt:"Welchem Land geh\u00f6rt dieses Wappen?",desc:"Erkenne L\u00e4nder an ihrem Wappen",beta:true},
  {id:"slf",           icon:"\u{1F524}",title:"Stadt-Land-Fluss",  group:"pure_geo",prompt:"Nenne Stadt, Land und Fluss\u2026",    desc:"Das klassische Geografiespiel",  beta:true},
  {id:"timezone_jumper",icon:"\u23F0",title:"Zeitzonen-Jumper",       group:"new_modes",prompt:"Welche Zeitzone?",                            desc:"Meistere die Zeitzonen der Welt",                  comingSoon:true},
...
const MODE_CATS={
  ...
  comparisons:{label:"Vergleiche",icon:"\u2696\ufe0f",modes:["comp_area","comp_pop","comp_north","comp_gdp","comp_density","comp_elevation","comp_coast","comp_borders","comp_life","comp_age","comp_forest"],cost:0},
...
/* Phase 95: inline area (km²) + population (millions) — self-contained, no external files */
const COMP_AREA={
...
async function loadProfile(){
  ...
  /* Phase 93: Smart-Merge Kennzeichen — Cloud ∪ Lokal */
...
function startDailyChallenge(){
  ...
  Object.assign(S,{sc:0,st:0,bs:0,rd:0,correct:0,lid:null,ph:"playing",scoreSaved:false,sessionAnswers:[],newStamps:[],half_removed:false,freezeActive:false,queueExtra:[],askedLids:new Set(),gameStartTime:Date.now(),adShownThisGame:false});
  lq();
}
...
function render(){
  ...
  if(S.ph==="gameover"){
  if(!sbProfile?.is_premium&&!S.adShownThisGame){S.adShownThisGame=true;S.adModal=true;}
...
function renderCollectionScreen(){
  ...
  const _CHLP=c=>{const M={
    'Deutschland':{ph:'Kürzel (vorne), z.B. M, BOC',info:'Kürzel ganz links (vorne) = Landkreis/Stadt',tp:'de'},
    'Österreich':{ph:'z.B. W, BN, GD',info:'1–3 Buchst. links = Bezirk',tp:'de'},
    'Schweiz':{ph:'z.B. ZH, BE, GE',info:'2 Buchst. links = Kanton',tp:'de'},
    'Frankreich':{ph:'Département (rechts), z.B. 75',info:'Blauer Streifen rechts: 2-stellige Département-Nummer',tp:'fr'},
  ...
  const r=M[c]||{ph:'Kürzel eingeben',info:'',tp:'de'};
  ...
  const spotter=`<div class="album-spotter">
    <div style="display:flex;align-items:baseline;justify-content:space-between;margin-bottom:.4rem">
      ...
      <select class="spotter-country-sel" onchange="S.spotterCountry=this.value;S.albumCountry=this.value;localStorage.setItem('geoquest_pref_country',this.value);S.spotterMsg='';render()">
        ...
        ${(()=>{const _lc=countries.filter(c=>PLATES_DATA.some(p=>p.country===c&&p.region!=="Nationales Kennzeichen"));const _nc=countries.filter(c=>!_lc.includes(c));return(_lc.length?`<optgroup label="\u{1F5FA}\uFE0F Lokale Regionen">${_lc.map(c=>`<option value="${esc(c)}" ${sCountry===c?"selected":""}>${esc(c)}</option>`).join("")}</optgroup>`:"")+(_nc.length?`<optgroup label="\u{1F30D} Ganzes Land">${_nc.map(c=>`<option value="${esc(c)}" ${sCountry===c?"selected":""}>${esc(c)}</option>`).join("")}</optgroup>`:"");})()}
      </select>
    </div>
...
function checkGridCriterion(name,crit){
  ...
  if(crit.type==="letter")return name[0]===crit.value;
...
function getGridSugg(txt){
  ...
  const used=S.gridData?S.gridData.usedCountries:new Set();
  return COUNTRIES.filter(c=>c.c.toLowerCase().startsWith(t)&&!used.has(c.c)).map(c=>c.c).slice(0,8);
}
...
function initLogikGitter(){
  ...
  const crit=GRID_CRIT.slice();
  for(let i=crit.length-1;i>0;i--){const j=~~(Math.random()*(i+1));[crit[i],crit[j]]=[crit[j],crit[i]];}
  let rowCrit,colCrit,tries=0;
  outer:while(tries++<40){
    rowCrit=crit.slice(0,3);colCrit=crit.slice(3,6);
    for(let r=0;r<3;r++){
      for(let c=0;c<3;c++){
        if(!allCo.some(co=>checkGridCriterion(co.c,rowCrit[r])&&checkGridCriterion(co.c,colCrit[c]))){
          crit.push(crit.shift());continue outer;
        }
      }
    }
    break;
  ...
  if(tries>=40){
    rowCrit=[GRID_CRIT[0],GRID_CRIT[1],GRID_CRIT[2]];
    colCrit=[GRID_CRIT[5],GRID_CRIT[6],GRID_CRIT[7]];
  }
...
function renderLogikGitter(sc){
  ...
  let gridHtml='<div class="lg-grid">';
  ...
  for(let r=0;r<3;r++){
    ...
    for(let c=0;c<3;c++){
      ...
      if(cell.country){
        gridHtml+=`<div class="lg-cell lg-filled">${flagOf(cell.country)}<div class="lg-cell-name">${esc(cell.country)}</div></div>`;
      }else if(gd.solved||gd.failed){
        gridHtml+='<div class="lg-cell lg-empty-done">\u2014</div>';
      }else{
        const act=isAct?' lg-active':'';
        gridHtml+=`<div class="lg-cell lg-empty${act}" data-r="${r}" data-c="${c}"
  ...
  if(gd.activeCell&&!gd.solved&&!gd.failed){
    ...
    const suggHtml=sugg.slice(0,6).map(s=>`<div class="lg-sugg-item" data-name="${esc(s)}" onclick="handleGridAnswer(S.gridData.activeCell.r,S.gridData.activeCell.c,this.dataset.name)">${flagOf(s)} ${esc(s)}</div>`).join("");
    inputHtml=`<div class="lg-inp-wrap">
      ...
      <input type="text" id="lg-inp" placeholder="Land eingeben\u2026" value="${esc(S.gridInput||'')}"
        oninput="S.gridInput=this.value;S.gridSugg=getGridSugg(this.value);render()"
...
const WAPPEN_DATA=[
  ...
  {cc:"at",c:"Österreich",img:"https://commons.wikimedia.org/wiki/Special:FilePath/Coat_of_arms_of_Austria.svg"},
...
function genWappenQ(){
  ...
  const _r=arr=>arr[\~~(Math.random()*arr.length)];
  ...
  const opts=[correct,...wrongs].sort(()=>Math.random()-.5).map(w=>w.c);
  return{type:"wappen",prompt:"Welchem Land gehört dieses Wappen?",subj:correct.c,img:correct.img,opts,ans:correct.c,lid:correct.cc,cc:correct.cc,meta:"🛡️ Wappen von "+correct.c};
}

/* ── STADT-LAND-FLUSS (SLF) ── (Phase 106) */
const SLF_LETTERS="ABCDEFGHIKLMNOPRSTW";
function initSLF(){
  clearInterval(tIv);
  const letter=SLF_LETTERS[\~~(Math.random()*SLF_LETTERS.length)];
...
function handleSLFSubmit(){
  ...
  const cityValid=startsOk(cityAns,L)&&CITIES.some(c=>norm(c.name)===cityAns||norm(c.asciiName)===cityAns);
  const countryValid=startsOk(countryAns,L)&&COUNTRIES.some(c=>norm(c.c)===countryAns||norm(displayCountry(c.c))===countryAns);
  const riverValid=startsOk(riverAns,L)&&(RIVERS_REAL.some(r=>norm(r.n)===riverAns)||['amazonas','rhein','donau','nil','niger','elbe','oder','weser','thames','seine','loire','rhone','tiber','ebro','tejo','vistula','oder','dnjepr','wolga','ural'].includes(riverAns.toLowerCase()));
...
function renderStadtLandFluss(sc){
  ...
  if(phase==="result"){
    ...
    return`<div class="scr"><div class="panel">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:.75rem">
        <div><div style="font-size:.6rem;letter-spacing:1px;color:var(--text3);font-weight:700">ERGEBNIS</div><div style="font-size:1.4rem;font-weight:900">\u{1F524} ${letter}</div></div>
      ...
      <button class="btn-p" style="margin-top:.85rem" onclick="initSLF()">\u{1F524} Neue Runde</button>
  ...
  return`<div class="scr"><div class="panel">
    ...
    <div class="slf-field"><label class="slf-label">\u{1F3D9} STADT</label><input id="slf-city" type="text" class="slf-input" placeholder="Stadt mit ${letter}…" autocomplete="off" autocorrect="off" value="${esc(answers.city)}" oninput="S.slfData.answers.city=this.value;render()" onkeydown="if(event.key==='Enter')document.getElementById('slf-country')?.focus()"></div>
    <div class="slf-field"><label class="slf-label">\u{1F30D} LAND</label><input id="slf-country" type="text" class="slf-input" placeholder="Land mit ${letter}…" autocomplete="off" autocorrect="off" value="${esc(answers.country)}" oninput="S.slfData.answers.country=this.value;render()" onkeydown="if(event.key==='Enter')document.getElementById('slf-river')?.focus()"></div>
    <div class="slf-field"><label class="slf-label">\u{1F4A7} FLUSS</label><input id="slf-river" type="text" class="slf-input" placeholder="Fluss mit ${letter}…" autocomplete="off" autocorrect="off" value="${esc(answers.river)}" oninput="S.slfData.answers.river=this.value;render()" onkeydown="if(event.key==='Enter')handleSLFSubmit()"></div>
...
/* ── ACCOUNT LÖSCHEN (DSGVO) ── (Phase 106) */
async function doDeleteAccount(){
  ...
  try{
    ...
    await sb.auth.admin?.deleteUser(sbUser.id).catch(()=>{});
    await sb.auth.signOut();
    showToast("\u{1F5D1}️ Konto gelöscht");
    S.settingsModal=false;
    doLogout();
  }catch(e){showToast("⚠️ Fehler: "+e.message);}
}

/* ── AD INTERSTITIAL ── (Phase 107) */
function showAd(){if(\!sbProfile?.is_premium){S.adModal=true;render();}}
...
function renderHomeTab(){
  ...
  const _homeHTML=`${_hdr}${renderDailyHero()}
    ...
    <div style="display:flex;align-items:center;gap:6px;margin-bottom:4px"><span style="font-size:.65rem;color:var(--text3);font-weight:700;letter-spacing:.8px">SCHWIERIGKEIT</span><span title="Casual: Große Städte, 12s, Wiederholung bei Fehler&#10;Hardcore: Alle Städte, 8s, kein Fehler&#10;Survival: Unbegrenzt, 1 Fehler = Ende" style="font-size:.72rem;cursor:help;color:var(--text3)">ℹ️</span></div>
...
function renderMehrTab(){
  ...
  </div><div style="text-align:center;padding:.75rem 0 .25rem;font-size:.62rem;color:var(--text3);line-height:1.9"><strong style="color:var(--text2)">GeoQuest</strong> &mdash; Das Geografie-Quiz<br><a href="mailto:kontakt@geoquest.app" style="color:var(--text3);text-decoration:none">kontakt@geoquest.app</a><br><span style="cursor:pointer;text-decoration:underline" onclick="showToast(\'Impressum folgt in Kürze\')">Impressum</span> &middot; <span style="cursor:pointer;text-decoration:underline" onclick="showToast(\'Datenschutz: Keine Weitergabe persönlicher Daten an Dritte.\')">Datenschutz</span></div>`;\n}\n\n/* LEADERBOARD helper (used from home) */
...
.mode-desc{color:var(--text3);font-size:.58rem;margin-top:3px;line-height:1.25;font-weight:400}
...
.lg-cell.lg-empty{color:var(--text3);font-size:1.4rem;font-weight:300;cursor:pointer}
